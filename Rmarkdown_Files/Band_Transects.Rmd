...
bands_Data$SiteName <- factor(bands_Data$SiteName, levels = unique(siteInfo1$SiteName))
...
print(a)
```
```{r Keyhole Wakame, echo=FALSE, fig.width=12, fig.height=7}
...
p1 <- ggplot(bands_KH_Up_p1,
             aes(x = Date, y = Mean_Density, color = SiteName, linetype = SiteName)) + 
  geom_line(size = 1) +
  scale_x_date(date_labels = "%Y", breaks = unique(bands_KH_Up_p1$Date),
               expand = expansion(mult = c(0.01, .01))) +
  scale_y_continuous(limits = c(0, bands_KH_Up_p1$Max_Mean), 
                     expand = expansion(mult = c(0, .1))) +
...
```
```{r Band Transect Plots, echo=FALSE, fig.width=12, fig.height=15}
for (i in levels(bands_Drop_Special_Cases$CommonName)) {
  ...
  p1 <- ggplot(bands_split$`San Miguel Island`,
    ...
    scale_x_date(date_labels = "%Y", date_breaks = '1 year',
                 expand = expansion(mult = c(0.01, .01))) +
    scale_y_continuous(limits = c(0, bands_split$`San Miguel Island`$Max_Mean), 
                       expand = expansion(mult = c(0, .1))) +
