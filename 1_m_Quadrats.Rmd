...
oneM_Data <- read_csv("Tidy_Data_Dont_Touch/oneM_Summary.csv") %>% 
...
oneM_Data$SiteName <- factor(oneM_Data$SiteName, levels = SiteLevels)
oneM_Data$CommonName <- factor(oneM_Data$CommonName, levels = unique(oneM_Data$CommonName))
oneM_Drop_Special_Cases <- oneM_Data %>% 
  dplyr::filter(ScientificName != "Undaria pinnatifida",
                ...
                CommonName != "southern sea palm, all",
                CommonName != "oar weed, all",
                CommonName != "California sea palm, all") %>% 
  base::droplevels()

oneM_KH_Up <-oneM_Data %>% 
  subset(SiteCode == 'KH' & ScientificName == 'Undaria pinnatifida')%>%
  droplevels() %>% 
  dplyr::mutate(Max_Mean = max(Mean_Density))

oneM_KH_Up_p2 <- oneM_KH_Up %>% 
  filter(CommonName == "wakame, juvenile")

oneM_KH_Up_p1 <- oneM_KH_Up %>% 
  filter(CommonName == "wakame, adult")

```
# Appendix `r One_Meter_Quad_Appendix_Letter`. 1 m Quadrat Data
...
```{r Keyhole Wakame 1 m, echo=FALSE, fig.height=7, fig.width=12}
p1 <- ggplot(oneM_KH_Up_p1,
             aes(x = Date, y = Mean_Density, color = SiteName, linetype = SiteName)) + 
  geom_line(size = 1) +
  scale_x_date(date_labels = "%Y", breaks = unique(oneM_KH_Up_p1$Date),
               expand = expansion(mult = c(0.01, .01))) +
  scale_y_continuous( # limits = c(0, oneM_KH_Up_p1$Max_Mean),   # Un-comment to make scales fixed across both graphs
                     expand = expansion(mult = c(0, .1))) +
  labs(title = oneM_KH_Up_p1$ScientificName,  
       subtitle = oneM_KH_Up_p1$CommonName,
...
p2 <- p1 %+% oneM_KH_Up_p2 +
  labs(title = NULL, x = "Year",
       subtitle = oneM_KH_Up_p2$CommonName) +
  theme(axis.title = element_text(hjust = .5, size = 18),
...
```
```{r 1 m Quadrats Plots, echo=FALSE, fig.height=16, fig.width=12}
for (i in levels(oneM_Drop_Special_Cases$CommonName)) {
  ...
  p1 <- ggplot(oneM_split$`San Miguel Island`,
    ...
    scale_y_continuous( # limits = c(0, oneM_split$`San Miguel Island`$Max_Mean),   # Un-comment to make scales fixed across both graphs 
                       expand = expansion(mult = c(0, .1))) +
