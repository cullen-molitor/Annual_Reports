...
SMI <- get_googlemap(c(-120.38, 34.05),
...
{ # SMI   ----
  SM <- ggmap(SMI, extent = "device") +
    ...
    geom_label(data = siteInfo1, hjust = .5, vjust = 1, size = 5,
               aes(x = Longitude, y = Latitude - .002, label = SiteCode)) +
    geom_sf_label(data = mpa, inherit.aes = FALSE, hjust = .5, vjust = .5, 
                  aes(label = NAME), size = 6) +
    scale_fill_manual(values = MPA_Colors) +
    scale_color_manual(values = MPA_Colors) +
    labs(title = "San Miguel",
         x = NULL, y = NULL) + 
    theme_get() 
  
  png(filename = "Report_Maps/SMI.png", width = 1000, height = 1000, type = "cairo")
  SM
  dev.off()
}
