...
{ # Shannon Index Calculation and Analyses  ----
...
for(i in unique(heat_map$IslandName)) {
  ...
  ISA_Heatmap_Annotated <- ggpubr::annotate_figure(
    p,
    bottom = text_grob(paste(" Fig", fig_counter, ". Indicator Species Analysis (ISA) for sites at", i, ". ISA indicates species significantly associated with nMDS clustering groups.",
                             "\n",
...
```{r Fish Biomass Ratio , warning=FALSE, message=FALSE, fig.height=8.5, fig.width=11, fig.keep = FALSE}
...
boot_ratio = function(data, indices){
  ...
  return(ratio)
  #  +  mean(runif(1000, min = 0, max = 10^-100))) 
}
...
# for some reason I could NOT get this with filter
...
# Fish_Biomass_Boot$SurveyYear <- factor(
#     Fish_Biomass_Boot$SurveyYear, levels = unique(Fish_Biomass_Boot$SurveyYear))

for(y in unique(Fish_Biomass_Boot$SurveyYear)){
for(s in unique(Fish_Biomass_Boot$ScientificName)){
    d <- Fish_Biomass_Boot %>%
    ...
    Booted_Ratios <- add_row(Booted_Ratios, ScientificName = s, SurveyYear = y, Mean_Ratio = ci_boot$t0, CI_minus = ci_boot$percent[4], CI_plus = ci_boot$percent[5])
    #print(c(s, y))
}
  #Fish_Biomass_Boot <- droplevels(Fish_Biomass_Boot)
}
...
for (year in 2007:2019) {
  Fish_Ratios <- Booted_Ratios %>% 
    ...
    dplyr::arrange(Targeted, CI_plus) 
...
```{r Fish Biomass Ratio Plots, warning=FALSE, message=FALSE, fig.height=8.5, fig.width=11}
...
FR_Plot3 <- ggpubr::ggarrange(
  Fish.Ratio.2011, Fish.Ratio.2012, 
  ncol = 2,
  common.legend = TRUE, legend = "bottom", align = "hv")

FR_Plot4 <- ggpubr::ggarrange(
  Fish.Ratio.2013, Fish.Ratio.2014, 
  ncol = 2,
  common.legend = TRUE, legend = "bottom", align = "hv")

FR_Plot5 <- ggpubr::ggarrange(
  Fish.Ratio.2015, Fish.Ratio.2016, 
  ncol = 2,
  common.legend = TRUE, legend = "bottom", align = "hv")

FR_Plot6 <- ggpubr::ggarrange(
  Fish.Ratio.2017, Fish.Ratio.2018, 
  ncol = 2,
  common.legend = TRUE, legend = "bottom", align = "hv")

FR_Plot7 <- ggpubr::ggarrange(
  Fish.Ratio.2019, 
  ncol = 1,
  common.legend = TRUE, legend = "bottom", align = "hv")
...
FR_annotated5 <- ggpubr::annotate_figure(
  FR_Plot5, 
  bottom = ggpubr::text_grob(
    paste(" nMDS plots for community similarity from 2005-2019, grouped by island. Symbols represent reserve status, ellipses represent 95% confidence regions,",
          "\n",
...
FR_annotated6 <- ggpubr::annotate_figure(
  FR_Plot6, 
  bottom = ggpubr::text_grob(
    paste(" nMDS plots for community similarity from 2005-2019, grouped by island. Symbols represent reserve status, ellipses represent 95% confidence regions,",
          "\n",
...
FR_annotated7 <- ggpubr::annotate_figure(
  FR_Plot7, 
  bottom = ggpubr::text_grob(
    paste(" nMDS plots for community similarity from 2005-2019, grouped by island. Symbols represent reserve status, ellipses represent 95% confidence regions,",
          "\n",
